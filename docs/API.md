...
The `403 FORBIDDEN` documented above under The `Host` header is what the **origin** returns for a request whose `Host` it does not recognise — whether that request was dialed directly against the origin's own non-standard port or forwarded to it by a proxy.

Whether a bad `Host` ever reaches the origin is a property of the **listener's port**, not of the proxy product. **Tailscale Serve routes by `Host` on its plain-HTTP listener but by the TLS server name on its HTTPS listener**, so the one proxy this project ships behaves two different ways and a client must be prepared for both.

**Plain HTTP on `:80` — the proxy answers, but only for a `Host` it cannot resolve at all.** Serve matches the request's `Host` against its own serve configuration before it opens any connection to the origin, so a `Host` it does not recognise is answered by the proxy itself and never reaches the origin at all. A client dialing this path with such a `Host` observes a `404` carrying no `X-Error-Code` header, no `X-API-Version` header, and none of the `X-Voice-*` headers this specification otherwise guarantees — none of that is the origin's output, and the body is the proxy's own short plain-text notice rather than a JSON error envelope. This is invisible from the server side: the origin never sees the request, so its logs show nothing for it.

Serve's match on this listener is looser than the origin's own, so the `404` is not the only outcome a bad `Host` can produce here. Serve strips the port — any port, not only a standard one — and appends the tailnet's MagicDNS suffix when one is absent, so `your-device`, `your-device.your-tailnet.ts.net:80` and `your-device.your-tailnet.ts.net:9999` all resolve to the same serve entry and are forwarded. The origin strips only `:80` and `:443` and then matches exactly, so of those three only the second is accepted; the other two are forwarded by Serve and then rejected by the origin with its own `403 FORBIDDEN`, envelope and all. A bad `Host` yields the proxy's `404` only when Serve can resolve it to no entry whatever — otherwise the client sees the origin's rejection, exactly as it would on `:443`.

**HTTPS on `:443` — forwarded to the origin.** Serve routes on the server name negotiated during the TLS handshake and never consults the HTTP `Host` header for routing; it forwards that header to the origin unchanged. A bad `Host` therefore does reach the origin, which answers with its own `403 FORBIDDEN` — carrying the `X-Error-Code` and `X-API-Version` headers and the JSON error envelope, byte for byte what a client dialing the origin directly would receive for the same request. The origin does see the request, and does log it. Because Serve applies no `Host` gate of its own on this path, the origin's own `security.expectedHost` check is the only `Host` validation anywhere in the deployment; an operator who leaves that setting unset has none.

**A wrong server name on `:443` — no HTTP response at all.** A TLS handshake naming a hostname Serve holds no configuration or certificate for fails at the transport layer. There is no HTTP status to observe and none of the response shapes above apply.

Both `Host` outcomes are terminal, not retryable: a retry re-issues the same request against the same routing decision, so nothing server-side can ever make it succeed. Surface either to the operator as a `Host` misconfiguration.

This describes the deployment this project ships — Tailscale Serve fronting the origin — and is a property of that proxy's two listeners, not a promise about every reverse proxy a client might sit behind. Only the `:80` shape above is exercised by this repository's suite (see What the operator's reverse-proxy configuration must honour below). The `:443` and failed-handshake shapes, and Serve's own `Host` normalisation on `:80`, are recorded from observation against a live tailnet and are covered by no automated test here.
...
Verified automatically by this repository's own test suite (`test/api-spec-contract.test.js`'s stub-proxy tests): a real turn is forwarded through an in-process proxy and what the client observes is compared, byte for byte, against what the origin emitted for the same request. That stub reproduces route-by-`Host` forwarding and rejection and nothing else, which is the shape of Serve's plain-HTTP listener only — its HTTPS listener routes by TLS server name instead and is modelled by nothing here; it is not Tailscale Serve, so a green suite proves this document and the stub agree with each other, not that either agrees with a live Serve instance. The operator's reverse proxy must:
